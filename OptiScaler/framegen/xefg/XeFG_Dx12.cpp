                                ...
                                IDXGISwapChain** swapChain, bool readyToRelease)
{
    ...
    if (_swapChainContext == nullptr)
    {
        ...
        if (result == XEFG_SWAPCHAIN_RESULT_SUCCESS)
        {
            ...
            if (Config::Instance()->FGXeFGIgnoreMaxInterpolations.value_or_default())
            {
                State::Instance().xefgMaxInterpolationCount = 3;
                LOG_INFO("FGXeFGIgnoreMaxInterpolations=true, overriding max to 3 (4x)");
            }
        }
        else
        {
